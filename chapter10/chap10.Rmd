...
The pursuit of predictive accuracy has led to a multitude of computational developments, each of which are optimized for certain types of data and use cases. Let's take the seemingly simple case of classification. Below, we plot three distinct scenarios for a two-class problem: simple linear boundary, non-linear, and discontinuous. Of the techniques that we have covered, a *logistic regression* is a natural fit for the linear boundary. While the method provides an analytically convenient answer, the resulting decision boundary will clearly misrepresent the contours of more complex scenarios. As the true decision boundary begins to bend and jump, non-parametrics are a far more flexible solution, but at the cost of interpretability. 
*K-nearest neighbors* (kNN), for example, performs classification by finding to which class the majority of neighboring observations belong. Neighborhoods are determined by first calculating the distance between a given point and all other points, then then using the k-closest observations to "vote". Alternatively, *decision tree learning* and its variants such as *Random Forest* split a sample into finer more homogeneous partitions as defined by learned thresholds along input variables. 
...
You will no doubt have noticed that the algorithms mentioned sound quite exotic relative to the plain vanilla regressions in Chapters 8 and 9. There are hundreds of classification algorithms that are easily accessible using modern data science software and can accommmodate both regression and classification problems. In this chapter, we explore [] of these methods and how each have a role in prediction for public policy. 
...
#Calc importance
  ...
  kable(imp, 
          ...
          col.names = c("Variable", "Increase Node Purity"),
          row.names = FALSE)  %>%
