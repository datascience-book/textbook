...
As we step through each element of the comparison, it becomes apparent that Split B can more effectively isolate downed tree areas -- it would be a 100% hit rate in the *North+* partition. Split A, in contrast, would result in a 66% hit rate in the *North* partition. Indeed, the IG calculation confirms that Split B would yield twice as much information as Split A.
...
Decision trees are an incredible contribution to machine learning. They can conduct automated variable selection on any type of data type. From the recursive partitioning, each leaf node is a subsample that is defined using a set of conditional criteria, making it easy to articulate and define specific subpopulations. The splitting mechanism makes it possible to capture the intricate patterns in the data, like interactions and non-linearities, that will otherwise not be detected using classical methods like regression.  The implications are that two or more input variables can be blended together to find cells of activities that are otherwise overlooked.
