...
__Linkage Methods__. A key input into this process is the linkage method $m$, which dictates which observations are used in calculating distance between clusters. The choice of linkage method not only changes the composition of clusters, but also influences the processing time -- more exhaustive the linkage, more computationally intensive the process, the longer it will require to complete. 

The simplest is the *single linkage* or nearest neighbor in which the closest pair of two points, $x_i$ and $y_i$, from two different clusters are merge together. 

$$min(d(x_i, y_j))$$

*Complete Linkage*,  Distance between the two farthest points in two clusters

$$max(d(x_i, y_j))$$

*Average Linkage*. Average distance between all points

$$\sum_i^k \sum_j^l d(x_i, y_j)$$

*Centroid distance*. Distance between cluster centroids

$$max(d(x,y))$$
*Ward's Distance*. Clusters that minimize variance increase by using an ANOVA sum of squares overall all partitions

$$\frac{||\bar{x_k}-x_i||^2}{\frac{1}{N_k} \frac{1}{N_t}}$$




```{r, echo = FALSE, fig.cap = "Comparison of linkage methods."}
dis.mat <- dist(inputs, method = "euclidean")  
hc <- hclust(dis.mat, method = "ward.D")  
plot(hc, cex = 0.001, col = "grey", main = "Dendrogram") 
```
...
### DIY: Clustering of 311

311 paper ^[https://www.ncbi.nlm.nih.gov/pmc/articles/PMC5645100/]
