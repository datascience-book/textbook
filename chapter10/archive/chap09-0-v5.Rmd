...
#Random Forest 
  library(ranger)
   adatree <- ranger(factor(linear) ~ x1 + x2, data = examp, mtry = 2)
   adavaried <- ranger(factor(varied) ~ x1 + x2, data = examp, mtry = 2)
   adadiscont <- ranger(factor(discont) ~ x1 + x2, data = examp, mtry = 2)
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.height = 2, fig.cap = "(1) A linearly separable problem. (2) A sigmoid."}
#Margin Example
margin_size <- -0.2
set.seed(123)
...
par(mfrow = c(1,2), mar = c(0, 1.2, 0.9, 0))
  ...
  #Linear Boundary
  plot(df$x, df$y, col =vec, pch = 16, axes = F, xlab = "x1", ylab = "x2", main = "(1)")
  lines(df$x, df$z+0.2)
  ...
  plot(l$x, l$logit, xlab = "x", ylab = "F(x)", col = "orange", type = "l", main = "(2)")
...
```{r, echo = FALSE, results = 'asis', warning=FALSE, message=FALSE, fig.cap = "Coefficient table of four alternative logistic regression specifications."}
 library(stargazer)
