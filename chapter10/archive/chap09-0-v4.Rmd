...
The `fit` object captures all of the inner workings of the decision tree. For example, just plotting the fit object will show the full depth of the tree. For a better formatted tree diagram, we substitute `rpart.plot` library, which auto-formats the tree and color codes nodes based on the concentration of the target variable.

```{r, fig.height = 2.5, fig.cap = "Decision tree using default parameters."}
#Plot
  library(rpart.plot)
...
The `fit` object also captures the cross validated accuracies for each depth of the tree, which can be printed using the `printcp()` function. The *CP table* contains the `CP` parameter representing tree complexity,  the number of splits `nsplit`, the prediction error in the training data `rel error`, the cross-validation error `xerror`, and the standard error `xstd`. 
...
$$\text{Mean }F_1 = \sum_\text{i}^\text{k} (\frac{n_k}{n}F_{1k})$$
  ...
  for(k in classes){
    results <- rbind(results, 
                     data.frame(weight = sum(actual == k, na.rm = T)/length(actual),
                                precision = sum(predicted == k & actual == k)/sum(predicted == kk, na.rm = T), 
                                recall = sum(predicted == k & actual == kk, na.rm = T)/sum(actual == kk, na.rm = T)))
  }
...
1. Tune the decision tree to increase the mean F1-statistic by testing different windows for the average, maximum and standard deviation of `accel`.
...
#Search for most optimal number of input features
  fit.tune <- tuneRF(x = train[,c("avg50", "max50", "min50", "sd50")], y =  factor(train$activity), 
                     ntreeTry = 500, 
                     mtryStart = 1, 
                     stepFactor = 2, 
                     improve = 0.001, 
...
Normally, we can plug the tuned parameter back into the `randomForest()` method and re-train the algorithm, but it unnecessary in this case as the default model already uses the same parameters. When applied to the test set, we see that the mean F1-statistic is much improved to `r yhat <- predict(fit.rf, test); print(round(meanF1(test$activity, yhat), 3));` -- or a whole 10-percentage point increase.
