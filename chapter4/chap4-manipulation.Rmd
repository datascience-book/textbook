...
__Tidy Data__.  The ultimate goal of data retrieval and processing is to construct a data set that is ready for analysis, modeling and visualization.Tthis process, however, can occupy between 50% to 80% of a data scientist's time [(@lohr2014)]. Data needs to be processed into different forms depending on the use case. The number of tools available to *wrangle* data are many. But what if a standard framework could be applied to reduce the time burden and make data more actionable sooner. For one thing, we could follow the principles of *tidy data* laid out in @wickham2014: 
