...
```{r ch8 ols fit, echo = F, fig.cap = "Diagnosing fit: OLS is best (for minimizing MSE)"}
  ...
  geom_segment(
    aes(xend = x, yend = reg_lm$fitted.values),
    color = "grey70", size = 0.3
  ) +
  geom_abline(
...
Applying `tidy()` to the regressions results (produced by the `lm()` function) returns a much simpler set of regression results: the coefficient estimates (`estimate` column), the standard errors for the estimates (`std.error`), and the t statistics and p-values associated with testing the coefficients against a null hypothesis that the coefficients equal zero.
...
which says that the sale price depends upon an intercept $\beta_0$, square footage, age, and the year in which the home was built.
...
The interpretation of the estimated coefficients changes a bit when you are in a multiple linear regression setting. We now interpret the coefficients as the relationship between $y$ and $x_i$, _conditional_ on the other variables. What do we mean by 'conditional'? The estimate of the relationship comes from a model that includes a specific set of other variables. Thus, _conditional_ means we've estimated the relationship between $y$ and $x_i$ _after controlling for_ the other variables. Further, the interpretation of the coefficients requires that we hold the other variables constant.

For example, when we only had square footage and an intercept on the right-hand side, we interpreted the coefficient on `gross_square_feet` as _the price increases approximately $466 for each additional (gross) square foot._ But with age and year-of-sale on the right-hand side as well (multiple regression!), we now interpret the coefficient on `gross_square_feet` as _the price increases approximately $473 for each (gross) square foot, holding age and year-of-sale fixed_.

### Functional form matters

Just as it is important to carefully interpret coefficient estimates, it is also important to understand/remember the assumptions that our regression model makes. For example, in our multiple-regression model

$$ \text{Price} = \beta_0 + \beta_1 \text{Footage} + \beta_2 \text{Age} + \beta_3 \text{Year} + \varepsilon $$

we impose several assumptions—many relating to the *functional form* of the model.

The *function form* of a model describes the mathematical way we allow the variables to relate to each other—*e.g.*, should we only consider `Age` or should we also add squared `Age`? When we write down the functional form of a model, we generally want to weigh complexity, accuracy, and interpetability.^[Later, we'll also discuss the external validity of a model.] A very complex model may also be accurate, but it may be very hard to interpret. Further, as we will discuss later, complexity doesn't guarantee accuracy. Finally, we tend to think of the modeling assumptions more as simplifications/restrictions—*i.e.*, we're trying to build a decent model, as opposed to a model that perfectly describes reality.

Back to the actual model for housing prices. The function form in the model above assumes that the *effects* of square footage, age, and year-of-sale are each *linear* and *separable*. The *linearity* of the effects means that adding one more year to the age of a house has the same effect whether the house is 2 years old or 200 years old. The *separability* of the effects means that an additional year of age will have the same effect on price, regardless of how large the house is.

It is certainly possible that these modeling assumptions do not reasonably fit our situation. For example, the effect of an additional year of age may decrease as a house gets older. Or the effect of aging 


- The separable assumption
  - What about effects that are not explained by a simple line? Some effects may be a polynomial. Some effects may be discontinuous (indicators).
  - What if the effect of one variable depends upon the levels of other variables? Interactions...
  - What if the effects are not linear but more percentage-based? Transformationg...
