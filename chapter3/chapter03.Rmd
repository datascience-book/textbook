...
\vspace{12pt} 
```{R, ch3 logical operators continued, eval = FALSE}
...
temp.table <- read.csv("tables/ch3_tab_logical_operators.csv")
...
R's assignment operator `<-` exists solely for the task: assigning a value (on the right-hand side) to the name (on the left-hand side). If you are reading code out load,^[Yes, it does happen.] `object <- value` is typically read "*object* gets *value.*""
...
As you can see, we have a freedom in the names that can be used for the objects. However, we can mitigate the heartache of troubleshooting erroneous code by using common variable name conventions. Variable names should avoid (1) a number as the first character in the name (e.g. `"1variable"` is not a good idea) as this will not register as an object, (2) any spaces in the name (e.g. `"variable 1"`), (3) a name of an existing function. 
...
\vspace{12pt} 
```{R, ch3 sequence example, eval = FALSE}
# Old school
c(1, 2, 3, 4, 5, 6)
...
\vspace{12pt} 
```{R, ch3 matrix example, eval = FALSE}
...
\vspace{12pt} 
```{R, ch3 matrix ex by row, eval = FALSE}
...
\vspace{12pt} 
```{R, ch3 matrix ex types, eval = FALSE}
# Character
...
\vspace{12pt} 
```{R, ch3 matrix ex fill, eval = FALSE}
...
\vspace{12pt} 
```{R, ch3 data frame example, eval = FALSE}
data.frame(
...
\vspace{12pt} 
```{R, ch3 data frame ex multiples, eval = FALSE}
data.frame(
  some_letters = c("a", "A", "b", "B"),
  some_numbers = 1:2
)
...
\vspace{12pt} 
```{R, ch3 list ex naming, eval = FALSE}
list(
  el_1 = "apples",
  el_2 = c(T, F, T),
...
require(knitr)
temp.table <- read.csv("tables/ch3_tab_preferred_packages.csv")
pander::pander(temp.table, split.cell = 60, split.table = Inf, justify = "left",
...
```{r, echo = FALSE, warning=FALSE, message = FALSE, fig.cap = "Recommended functions to load an assortment of files."}
require(knitr)
temp.table <- read.csv("tables/ch3_tab_loading_functions.csv")
pander::pander(temp.table, split.cell = 60, split.table = Inf, justify = "left",
